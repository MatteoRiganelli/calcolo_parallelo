...
MPI_Request request2, request4;


int main(argc, argv)
int argc;
char *argv[];{
int n = 7; //Dimensione matrice
float s,tau;

float dati[10]; //Dati da inviare allo slave
    ...
    for (ip=1;ip<=n-1;ip++) {
        ...
        for (iq=ip+1;iq<=n;iq++) {
            ...
            if (i > 4 && (float)(fabs(d[ip])+g) == (float)fabs(d[ip]) && (float)(fabs(d[iq])+g) == (float)fabs(d[iq])) 
                ...
                //PRIMO FOR    
                if( ip-1 > size -1 ){//controlla che il numero di iterazioni sia maggiore del numero processi
                    dati[8] = 0;   //se si fai algoritmo allocazione dinamica altrimenti dai tutte le iterazioni ai processi disponibili
                    for (j=1;j<size;j++) {
                        dati[0] =j;
                ...
                //SECONDO FOR
                int indice=1;
                if( ((iq-1)-(ip+1) +1) > (size-1) )  { //controllo lavoro simile a prima
                    indice=ip+1; //j=ip+1;j<=iq-1;j++; NUM_ITERAZIONI
                    for (j=1;j<size;j++) {
                        dati[0] = indice;
                        indice++;
...
//Ricevi i dati dal master per fare le varie rotate
    while(1){
